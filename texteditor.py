...
heading = Label(text_window, text="Text Editor", font=('bold',20), bg='light grey')
heading.pack()

# Creates the scroll bar on the right side along with the area where the text will be
scroll = Scrollbar(text_window).pack(side=RIGHT, fill=Y)
editor = Text(text_window, width=400, height=450, yscrollcommand=scroll.set).pack(fill=BOTH)
scroll.config(command=editor.yview)

# The button where the user can save their text
button = Button(text_window, text='Save', font=('normal', 10), command=save, bg='yellow')
